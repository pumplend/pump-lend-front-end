...
const fetchPoolStakingData = async (_poolStakingData: PublicKey) => {
  try {
    const accountInfo = await connection.getAccountInfo(
      new PublicKey(_poolStakingData),
    );
...
const fetchUserStakingData = async (_userStakingData: PublicKey) => {
  try {
    const accountInfo = await connection.getAccountInfo(
      new PublicKey(_userStakingData),
    );
...
const fetchSystemConfigData = async (_systemConfig: PublicKey) => {
  try {
    ...
    const authority = new PublicKey(
      data.slice(offset + 1, offset + 33),
    ).toBase58();
    const poolTokenAuthority = new PublicKey(
      data.slice(offset + 33, offset + 65),
    ).toBase58();
    const pumpFunProgram = new PublicKey(
      data.slice(offset + 65, offset + 97),
    ).toBase58();
...
const solPriceFetch = async () => {
  try {
    return Number((await api_price_oracle("solana"))["solana"]["usd"]);
  } catch (e) {
    return 0;
  }
};
...
const culcuateInitCurveBorrowAbleAmount = (amount: number) => {
